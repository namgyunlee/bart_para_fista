...
void iter2_para_fista(const iter_conf* _conf,
	const struct operator_s* normaleq_op,
	unsigned int D,
	const struct operator_p_s* prox_ops[D],
	const struct linop_s* ops[D],
	...
	struct iter_monitor_s* monitor)
{
	...
	auto conf = CAST_DOWN(iter_para_fista_conf, _conf);
	...
	para_fista(conf->maxiter, eps * conf->tol, conf->INTERFACE.alpha * conf->step / maxeigen, size, select_vecops(image_adj),
		continuation, OPERATOR2ITOP(normaleq_op), OPERATOR_P2ITOP(prox_ops[0]), image, image_adj, monitor);
