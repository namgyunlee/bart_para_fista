...
static void ravine2(const struct vec_iter_s* vops, long N, float* ftp, float* xa, float* xb)
{
	float p = 1.f / 20.f;
	float q = 1.f / 2.f;
	float r = 4.f;
...
// Para-FISTA: Modified by Nam Gyun Lee 2023-05-22
/**
 * Parameterized Iterative Soft Thresholding/FISTA to solve min || b - Ax ||_2 + lambda || T x ||_1
 *
 * @param maxiter maximum number of iterations
 * @param epsilon stop criterion
 * @param tau (step size) weighting on the residual term, A^H (b - Ax)
 ...
 * @param b observations
 */
void para_fista(unsigned int maxiter, float epsilon, float tau,
	long N,
	const struct vec_iter_s* vops,
	ist_continuation_t ist_continuation,
	struct iter_op_s op,
	...
	struct iter_monitor_s* monitor)
{
	...
	for (itrdata.iter = 0; itrdata.iter < maxiter; itrdata.iter++) {
		...
		ravine2(vops, N, &ra, x, o); // Para-FISTA
		iter_op_call(op, r, x);      // r = A x
		vops->xpay(N, -1., r, b);    // r = b - r = b - A x
	...
	debug_printf(DP_DEBUG3, "\n");
	debug_printf(DP_DEBUG2, "\t\tPara-FISTA iterations: %u\n", itrdata.iter);
