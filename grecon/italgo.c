...
struct iter italgo_config(enum algo_t algo, int nr_penalties, const struct reg_s* regs, unsigned int maxiter, float step, bool hogwild, bool fast, const struct admm_conf admm, float scaling, bool warm_start)
{
	...
	switch (algo) {
        ...
        // Para-FISTA: Modified by Nam Gyun Lee 2023-05-22
		case ALGO_PARAFISTA: {

			debug_printf(DP_INFO, "Para-FISTA\n");

			assert(1 == nr_penalties);

			PTR_ALLOC(struct iter_para_fista_conf, pfconf);
			*pfconf = iter_para_fista_defaults;
			pfconf->maxiter = maxiter;
			pfconf->step = step;
			pfconf->hogwild = hogwild;

			PTR_ALLOC(struct iter_call_s, iter2_para_fista_data);
			SET_TYPEID(iter_call_s, iter2_para_fista_data);

			iter2_para_fista_data->fun = iter_para_fista;
			iter2_para_fista_data->_conf = CAST_UP(PTR_PASS(pfconf));

			italgo = iter2_call_iter;
			iconf = CAST_UP(PTR_PASS(iter2_para_fista_data));
